...
class HeadlessDetectionApp:
    ...
    def _find_available_model(self):
        ...
        # Modelos encontrados en tu sistema (priorizando los más nuevos)
        possible_paths = [
            "/home/jose/hailo-rpi5-examples/resources/yolov11s_h8l.hef",      # YOLOv11 small (más nuevo)
            "/home/jose/hailo-rpi5-examples/resources/yolov11n_h8l.hef",      # YOLOv11 nano (más rápido)
            "/home/jose/hailo-rpi5-examples/resources/yolov8s_h8l.hef",       # YOLOv8 small
            "/home/jose/hailo-rpi5-examples/resources/yolov8m_h8l.hef",       # YOLOv8 medium
            "/home/jose/hailo-rpi5-examples/resources/yolov6n_h8l.hef",       # YOLOv6 nano
            "/home/jose/hailo-rpi5-examples/resources/yolov5m_wo_spp_h8l.hef", # YOLOv5 medium
            "/home/jose/hailo-rpi5-examples/resources/yolov5n_seg_h8l.hef",   # YOLOv5 nano segmentation
        ]
    ...
    def create_v4l2_pipeline_with_correct_postproc(self):
        """Pipeline V4L2 con funciones de post-procesamiento optimizadas"""
        if self.post_process_so:
            # Usar la función más adecuada para cada modelo
            function_name = self.get_best_function_for_model(self.model_path)
            ...
            pipeline_str = f"""
                ...
                video/x-raw,format=RGB,width=640,height=640 ! 
                hailonet hef-path={self.model_path} ! 
                hailofilter function-name={function_name} so-path={self.post_process_so} ! 
                identity name=identity_callback ! 
                fakesink sync=false
            """
        else:
    ...
    def get_available_functions(self):
        """Obtener lista de funciones disponibles en la librería"""
        return ["yolov5", "yolov5m_vehicles", "yolov5m_vehicles_nv12", "yolov5_no_persons", 
                "yolov5s_nv12", "yolov5s_personface", "yolov8m", "yolov8s", "yolox"]
    
    def get_best_function_for_model(self, model_path):
        """Determinar la mejor función de post-procesamiento para cada modelo"""
        model_name = model_path.lower()
        
        if "yolov11" in model_name:
            # YOLOv11 usa arquitectura similar a YOLOv8
            if "yolov11s" in model_name or "yolov11n" in model_name:
                return "yolov8s"
            else:
                return "yolov8m"  # Para yolov11m o modelos más grandes
            ...
            return "yolov8s"  # Por defecto para YOLOv8
            
        elif "yolov5" in model_name:
            if "vehicles" in model_name:
                return "yolov5m_vehicles"
            elif "personface" in model_name:
                return "yolov5s_personface"
            else:
                return "yolov5"
            ...
            # Por defecto, usar yolov8s para modelos nuevos
            return "yolov8s"
    ...
    def create_pipeline(self):
        ...
        for attempt_name, pipeline_func in pipeline_attempts:
            try:
                ...
                # Extraer y mostrar la función de post-procesamiento si existe
                if "function-name=" in pipeline_str:
                    func_start = pipeline_str.find("function-name=") + 14
                    func_end = pipeline_str.find(" ", func_start)
                    if func_end == -1:
                        func_end = pipeline_str.find("!", func_start)
                    function_name = pipeline_str[func_start:func_end]
                    print(f"   📋 Función post-proc: {function_name}")
